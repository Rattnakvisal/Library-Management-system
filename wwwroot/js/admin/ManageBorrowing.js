...
function initializeBookCodeBindings() {
    ...
    bindCodeToTitle(
        "#edit-borrowing-bookCodeSelect",
        "#edit-borrowing-bookTitleInput",
    );
    bindCodeToTitle(
        "#edit-reservation-bookCodeSelect",
        "#edit-reservation-bookTitleInput",
    );
}
...
function upsertBookCodeOption(select, code, title) {
    ...
    const existing = select.find("option").filter(function () {
        return (
            ($(this).val() || "").toString().trim().toLowerCase() ===
...
function initializeEditReservationButton() {
    $(".reservation-book-table").on(
        ...
        function () {
            ...
            $("#edit-reservation-reservationDateInput").val(
...
function initializeAddBorrowingButton() {
    $("#confirmBorrowing").on("click", async function () {
        ...
        if (!username || !bookCode) {
            await showAlert(
                "Missing Information",
                "Username and book code are required.",
                "warning",
                "#f59e0b",
            );
            return;
        ...
        try {
            const payload = await postForm(
                "/admin/manageborrowingbook/borrowing/create",
                formData,
            );
            await showAlert(
                "Success",
                payload.message || "Borrowing created successfully.",
                "success",
                "#16a34a",
            );
...
function initializeUpdateBorrowingButton() {
    $("#updateBorrowing").on("click", async function () {
        ...
        const bookCode = ($("#edit-borrowing-bookCodeSelect").val() || "")
            .toString()
            .trim();
        ...
        try {
            const payload = await postForm(
                `/admin/manageborrowingbook/borrowing/update/${id}`,
                formData,
            );
            await showAlert(
                "Success",
                payload.message || "Borrowing updated successfully.",
                "success",
                "#16a34a",
            );
            window.location.reload();
...
function initializeUpdateReservationButton() {
    $("#updateReservation").on("click", async function () {
        ...
        const bookCode = ($("#edit-reservation-bookCodeSelect").val() || "")
            .toString()
            .trim();
        const reservationDate = $(
            "#edit-reservation-reservationDateInput",
        ).val();

        if (!id || !bookCode) {
            await showAlert(
                "Missing Information",
                "Reservation id and book code are required.",
                "warning",
                "#f59e0b",
            );
            return;
        ...
        try {
            const payload = await postForm(
                `/admin/manageborrowingbook/reservation/update/${id}`,
                formData,
            );
            await showAlert(
                "Success",
                payload.message || "Reservation updated.",
                "success",
                "#16a34a",
            );
            window.location.reload();
...
function initializeProcessBorrowingButton() {
    $(".borrowing-book-table").on(
        "click",
        ".process-borrowing-btn",
        function () {
            ...
            Swal.fire({
                ...
                try {
                    const payload = await postNoBody(
                        `/admin/manageborrowingbook/borrowing/return/${id}`,
                    );
                    await showAlert(
                        "Returned",
                        payload.message || "Book marked as returned.",
                        "success",
                        "#16a34a",
                    );
                    window.location.reload();
...
function initializeMarkFinePaidButton() {
    $(".borrowing-book-table").on("click", ".mark-fine-paid-btn", function () {
        const id = ($(this).data("borrowing-id") || "").toString().trim();
        const fineAmount = ($(this).data("fine-amount") || "0.00")
            .toString()
            .trim();
        ...
        Swal.fire({
            ...
            try {
                ...
                const payload = await postForm(
                    `/admin/manageborrowingbook/fine/mark-paid/${id}`,
                    formData,
                );
                await showAlert(
                    "Fine Paid",
                    payload.message || "Fine marked as paid.",
                    "success",
                    "#16a34a",
                );
                window.location.reload();
    ...
    $(".reservation-book-table").on(
        ...
        function () {
            ...
            Swal.fire({
                ...
                try {
                    const payload = await postNoBody(
                        `/admin/manageborrowingbook/reservation/approve/${id}`,
                    );
                    await showAlert(
                        "Approved",
                        payload.message || "Reservation approved.",
                        "success",
                        "#16a34a",
                    );
                    window.location.reload();
        ...
        function () {
            ...
            Swal.fire({
                ...
                try {
                    ...
                    const payload = await postForm(
                        `/admin/manageborrowingbook/reservation/reject/${id}`,
                        formData,
                    );
                    await showAlert(
                        "Rejected",
                        payload.message || "Reservation rejected.",
                        "success",
                        "#16a34a",
                    );
                    window.location.reload();
