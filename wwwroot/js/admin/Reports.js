(function ($) {
    ...
    function setSelectButtonText($button, text) {
        $button.html(
            "<span>" +
                escapeHtml(text) +
                '</span><i class="bi bi-chevron-down caret"></i>',
        );
    }
    ...
    function renderPagination(currentPage, totalPages, totalRows) {
        ...
        var startIndex =
            safeTotalRows === 0
                ? 0
                : (safeCurrentPage - 1) * state.pageSize + 1;
        var endIndex =
            safeTotalRows === 0
                ? 0
                : Math.min(safeCurrentPage * state.pageSize, safeTotalRows);
        $("#reportMeta").text(
            "Showing " +
                startIndex +
                "-" +
    ...
    async function loadReport() {
        ...
        try {
            ...
            if (!response.ok || !payload.success) {
                throw new Error(
                    payload.message || "Failed to load report data.",
                );
            }
    ...
    $(function () {
        setSelectButtonText(
            $("#reportTypeBtn"),
            getReportConfig(state.reportType).title,
        );
        setSelectButtonText($("#exportTypeBtn"), "Export Type");
